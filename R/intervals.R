...
calculate_aggregate_trim_sym <- function(team_cdfs, n_trim){
  ...
  trim <- team_cdfs %>%
    ...
    dplyr::summarise(value = mean(value[value>(sort(value)[n_trim]) & value<=(sort(value)[n_models-n_trim])]))
...
calculate_aggregate_trim_ext <- function(team_cdfs, n_trim){
  ...
  trim_ext <- team_cdfs %>%
    dplyr::group_by(quantile) %>%
    dplyr::filter(quantile %in% c(5, 25, 50)) %>%
    dplyr::summarise(value= mean(sort(value)[(n_trim+1):n_models]))
  trim_ext <- rbind(trim_ext,
                    team_cdfs %>%
                      dplyr::group_by(quantile) %>%
                      dplyr::filter(quantile%in% c(50,75, 95)) %>%
                      dplyr::summarise(value= mean(sort(value)[1:(n_models-n_trim)])))
  trim_ext <- trim_ext %>%
    dplyr::group_by(quantile) %>%

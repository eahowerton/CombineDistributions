...
This vignette implements the second case study from [@EAH-ms]. We use predictions from a real-time multi-model effort to evaluate the efficacy of stay-at-home interventions early in the COVID-19 pandemic (May-November 2020). SAY MORE ABOUT THE DATA? Details on the study and the data can be found in REF. 
...
First, we load the data (both individual model projections and observations). **These data are provided as in the package??**

```{r load-proj}
# individual projections
projs <- read_csv("/Users/eah394/Documents/GitHub/aggregation_methods/data/mmods_data/team_submissions_incl_agg_round2_allquantiles.csv")
# rm aggregate row -- for now
projs <- projs %>%
  filter(id != "aggregate", objective == "cumu_deaths", intervention == "closed")

# observations
obs <- read_csv("/Users/eah394/Documents/GitHub/aggregation_methods/data/mmods_data/county_deaths_closed_partial.csv") %>% 
  dplyr::select(cumu_deaths, Population.x) %>% 
  dplyr::mutate(obs_num = 1:n())
...
p1 <- projs %>%
  ...
  filter(quantile %in% c(5,25,50,75,95)) %>%
  ...
  ggplot() +
  geom_vline(data = data.frame(value = quantile(obs$cumu_deaths, 0.5),
                               quant = 50), 
             aes(xintercept = value,  color = "darkgrey"), size = 0.3)+
  geom_rect(data = data.frame(xmin = quantile(obs$cumu_deaths, 0.05),
                              xmax = quantile(obs$cumu_deaths, 0.95),
                              ymin = -Inf, ymax = Inf), 
...
First, we define a `data.frame` with all possible aggregation methods to try.

```{r}
# define methods to test
test_trim <- expand.grid(n_trim = c(NA,1:16), 
                         method = c("LOP", "vincent"),
...
aggs <- foreach(i=1:nrow(test_trim)) %dopar% {
  aggregate_cdfs(projs %>% mutate(quantile = quantile/100),
                 id_var = "id", 
...
p2a_inset = aggs %>% 
  filter(int_ext == "exterior") %>%
  ggplot()+
  geom_vline(data = data.frame(x = median(obs$cumu_deaths)),
             aes(xintercept = x, color = "darkgrey"), size = 0.3)+
  geom_rect(data = data.frame(xmin = quantile(obs$cumu_deaths, 0.05),
                              xmax = quantile(obs$cumu_deaths, 0.95),
                              ymin = -Inf, ymax = Inf), 
...
obs %>%
  ggplot(aes(x = Population.x, y = cumu_deaths)) +
  geom_point() +
...
scores_CRPS <- crossing(aggs,obs %>% dplyr::select(cumu_deaths) %>% 
                          mutate(obs_num = 1:nrow(obs)))
# set as data.table
setDT(scores_CRPS)
# get CRPS score
...
pdfs <- pdfs[, .(dens_x = density(samp)$x, 
             ...
             by = .(method, n_trim, int_ext)]
scores_log <- crossing(pdfs, obs %>% dplyr::select(cumu_deaths) %>% mutate(obs_num = 1:nrow(obs)))
scores_log <- setDT(scores_log)[, .(logS = log_score(dens_x, dens_y, cumu_deaths)), 
                   by = .(method, n_trim, int_ext, obs_num, cumu_deaths)]

# kullback-leibler
scores_klic <- pdfs[, .(klic = KLIC(dens_x, dens_y, obs$cumu_deaths)),
                    by = .(method, n_trim, int_ext)]
  
# coverage
scores_cov <- aggs[, .(cov90 = cov(quantile, value, obs$cumu_deaths, 0.9)), 
                   by = .(method, n_trim, int_ext)]
...
Now we can plot the outcomes of this performance analysis.
...
ms_vals <- list(
  ind_mod = list(
    min_median = min(projs %>% filter(quantile == 50) %>% pull(value)),
    max_median = max(projs %>% filter(quantile == 50) %>% pull(value)),
    min_90_PI = min(projs %>% group_by(id) %>% summarise(range = value[quantile == 95] - value[quantile == 5])  %>% pull(range)),
    max_90_PI = max(projs %>% group_by(id) %>% summarise(range = value[quantile == 95] - value[quantile == 5])  %>% pull(range))
  ),
  aggs = list(

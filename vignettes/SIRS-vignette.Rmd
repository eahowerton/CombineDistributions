...
We vary the parameters in the SIRS model to represent four distinct models with varying assumptions about the transmission process. There are two types of uncertainty that these models encapsulate: 
...
To implement these cases, we create a `data.frame` of parameter sets to consider.
...
# scenario 1: none
none <- data.frame(beta = beta, gamma = gamma,rho = rho_none, 
                   model = sort(rep(LETTERS[1:4], n_samples)),
                   struc_uncert = "none")
# scenario 2: between
btn <- data.frame(beta = beta, gamma = gamma, rho = rho_btn, 
                  model = sort(rep(LETTERS[1:4], n_samples)),
                  struc_uncert = "btn")
# scenario 3: within
win <- data.frame(beta = beta, gamma = gamma, rho = rho_win, 
                  model = sort(rep(LETTERS[1:4], n_samples)),
                  struc_uncert = "win")
...
We run the simulations for all the parameter sets we've defined. To do so, we need to define the length of the simulation (52 weeks), population size (1000 individuals), and initial conditions (1% infected, all else susceptible).
...
# define facet labels
labs_struc <- c("atop(Structural~uncertainty~bold(not~captured),all~models~assume~unlikely~reinfection)", 
               "atop(Structural~uncertainty~bold(captured~between~models),A~and~C~asume~unlikely~B~and~D~assume~likely~reinfection)", 
               "atop(Structural~uncertainty~bold(captured~within~models),all~models~consider~unlikely~and~likely~reinfection)")
names(labs_struc) <- c("none", "btn", "win")


ub_labs <- list(bquote(A:mu[beta]==.(m1_beta)),
                bquote(B:mu[beta]==.(m2_beta)),
                bquote(C:mu[beta]==.(m3_beta)),
                bquote(D:mu[beta]==.(m4_beta)))

  
labs_struc_simp = c("Structural~uncertainty~bold(not~captured)",
                    "Structural~uncertainty~bold(captured~between~models)",
                    "Structural~uncertainty~bold(captured~within~models)")
...
p1_cumu <- pdfs %>% 
  ...
  mutate(struc_uncert = factor(ifelse(struc_uncert == "none", labs_struc[1], ifelse(struc_uncert == "btn", labs_struc[2], labs_struc[3])), levels = labs_struc)) %>%
  ggplot(aes(x = x, y = y)) +
  geom_path(aes(color = as.factor(model)), size= 0.5) +
  facet_grid(cols = vars(struc_uncert), labeller = label_parsed) +
...
plot_grid(p1_cumu, p1_peak + theme(legend.position = "none", strip.text = element_blank()), ncol = 1)
```
...
Next we test how well each of these aggregation methods perform, given a range of assumptions about the future. We use the continuous rank probability score (CRPS)
...
We summarise the results based on a few key intervals. 
...
We combined these plots to generate Figure 3 and 4 in [@EAH-ms]. 

### Figure 3

```{r Fig3, echo = FALSE, fig.width=12, fig.height = 10}
plot_grid(
  p1_cumu + 
    labs(subtitle = "A: Individual model PDFS"), 
  p2_cumu + 
    labs(subtitle = "B: Aggregate PDFs"), 
  p3_cumu + 
    labs(subtitle = "C: Relative performance of aggregates"), 
  ncol = 1
)

ggsave("/Users/eah394/Documents/GitHub/aggregation_methods/figures/general_sir/pdfs_scores_cum.pdf", width = 12, height = 10 , units = "in")
```


### Figure 4

```{r Fig4, echo = FALSE, fig.width=13, fig.height = 4}
...
F4a = p1_peak + 
  geom_path(data = agg_pdfs %>% 
            ...
            aes(color =  method), size = 1.5) +
  labs(subtitle = "A: Individual model and aggregate PDFs")+
  scale_color_manual(values = c(colrs_mod, colrs_agg), labels = c(ub_labs, "LOP", "Vincent")) 
  
  

F4b = diff_quant %>% 
  filter(variable == "peak_cases", struc_uncert == "none", true_rho == "lik") %>%
  mutate(struc_uncert = factor(ifelse(struc_uncert == "none", 
                                                  labs_struc_simp[1], 
                                                  ifelse(struc_uncert == "btn", 
                                                         labs_struc_simp[2], 
  ...
  ggplot() +
  geom_vline(aes(xintercept = 0), color = "grey") +
  geom_text(data = data.frame(x = c(0.5,-0.5),
                              y = c(0.5,0.5),
                              lab = c("LOP performs better", "Vincent performs better"),
                              hjust = c(0,1),
                              obs_rho = 26),
            aes(x = x, y = y, label = lab, hjust = hjust, color = lab), size = 1.8) +
  geom_segment(aes(x = lower_25, y = plot_y, xend = upper_75, yend = plot_y), size = 1.5) +
  geom_pointrange(aes(x = med, y = plot_y, xmin = lower, xmax = upper)) +
  facet_grid(cols = vars(struc_uncert), rows =  vars(true_rho), switch = "y", 
             labeller = labeller(true_rho = labs_rho, struc_uncert = label_parsed)) +
  labs(x = "CRPS(Vincent) - CRPS(LOP)", subtitle = "B: Relative performance of aggregates") +
  scale_color_manual(values = colrs_agg) +
  scale_y_reverse(breaks = labs_m, labels = names(labs_m)) +
  theme_bw()+
  theme(axis.title.y = element_blank(),
        legend.position = "none",
        panel.grid = element_blank(),
...
plot_grid(plot_grid(F4a, F4b, nrow = 1, align = "vh", axis = "tb", rel_widths = c(0.7, 0.3)))
ggsave("/Users/eah394/Documents/GitHub/aggregation_methods/figures/general_sir/pdfs_scores_peak.pdf", width = 13, height = 4 , units = "in")
```

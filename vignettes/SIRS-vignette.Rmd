...
```{r plot-model-projections, fig.width=9, fig.height = 4, echo = FALSE}
# reorder factor 
...
```{r plot-aggs, fig.width=9, fig.height = 4, echo = FALSE}
...
# define true waning
obs_params <- rbind(data.frame(beta = obs_beta, gamma = obs_gamma, rho = ifelse(rho_likely == 0, 0, 1/rho_likely), 
                               true_beta = sort(rep(c(paste0("m", 1:4), "mean"), n_obs)),
                               true_rho = "Y"),
                    data.frame(beta = obs_beta, gamma = 1/obs_gamma, rho = ifelse(rho_unlikely == 0, 0, 1/rho_unlikely), 
                               true_beta = sort(rep(c(paste0("m", 1:4), "mean"), n_obs)),
                               true_rho = "N"))
...
We use the continuous rank probability score (CRPS) to assess performance of the aggregate distributions against these synthetic future observations.
...
ggsave(paste0(fig_path,"pdfs_scores_cum.pdf"), width = 10, height = 7 , units = "in")
```
...
ggsave(paste0(fig_path,"pdfs_scores_peak.pdf"), width = 10, height = 3 , units = "in")
```
